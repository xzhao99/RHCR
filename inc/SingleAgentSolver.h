...
public:
    ...
    // int map_size;
    double path_cost;
    double min_f_val;  // std::min f-val seen so far
	...
	double compute_h_value(const BasicGraph& G, int curr, int goal_id,
		const std::vector<std::pair<int, int> >& goal_location) const;

    virtual Path run(const BasicGraph& G, const State& start, const std::vector<std::pair<int, int> >& goal_location, ReservationTable& RT) = 0;
